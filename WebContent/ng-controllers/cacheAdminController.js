		...
		$scope.doInitRequest = 
			function($http) {
				...
				$http.get(url).success(function(jsonResponse) {
					var cityPrototype = $scope.deepDecodeJSON(jsonResponse);
					$scope.extractPrototypes(cityPrototype);
				});
				...
				url = "http://" + window.location.hostname + ":9090/argame/city-caches";
				$http.get(url).success(function(jsonResponse) {
					// Extract city list
					var cities = $scope.deepDecodeJSON(jsonResponse);
					$scope.browserList[0] = cities;
					$scope.addMissingPrototypeFields(cities);
